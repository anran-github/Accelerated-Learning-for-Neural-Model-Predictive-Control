...
class NOM_IPP(nn.Module):
    def __init__(self,hidden_obj=256,input_num=6, obj_fun=0, constrains=[]):
...
if __name__ == '__main__':
    ...
    for t in i_bar:
        ...
        # # Adt= torch.tensor([[xt[0,0]+0.1*xt[1,0]],[-0.1*9.8*torch.sin(xt[0,0]) + 1*xt[1,0]]]).to(device)
        # xtt =  xt + 0.1*(torch.tensor([[xt[1,0]],[9.8*torch.sin(xt[0,0])]])) + Bd @ optimal_u.reshape(1,1)
