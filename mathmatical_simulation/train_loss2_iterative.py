...
parser.add_argument('--pre_trained', type=str, default='weights/loss2_only_weight_0.0_1.0_0.0.pth',help='input your pretrained weight path if you want')
...
for repeat_i in range(repeat_cnt):
    ...
    if len(args.pre_trained):
        ...
        # test PI
        if repeat_i == 0: # only on the first iteration
            lowest_PI, highest_in = test_performance_index(model, device=device, xr=0.0)
            lowest_PI_num_ratio = lowest_PI/highest_in
    ...
    def test(model, test_loader,epoch):
        ...
        if lowest_PI_num_ratio > (test_PI/num_in):
            ...
            torch.save(model.state_dict(), os.path.join(
                save_path,'loss2_iter_weight_{}_{}_{}.pth'.format(
...
test_performance_index(model, device=device, xr=0.0, model_path=os.path.join(save_path,'loss2_iter_weight_{}_{}_{}.pth'.format(designed_weights[0], designed_weights[1], designed_weights[2])))
