...
x_train_full = X_train_tensor.to(device)
y_train_full = y_train_tensor.to(device)

def closure():
    optimizer.zero_grad()
    outputs = model(x_train_full)
    loss1, loss2, loss3 = loss_function(x_train_full, outputs, y_train_full)
    loss = loss1 + loss2 + loss3
    loss.backward()
    print(f'Closure - Loss1: {loss1.item():.4f}, Loss2: {loss2.item():.4f}, Loss3: {loss3.item():.4f}')
    return loss
...
optimizer = torch.optim.LBFGS(model.parameters(), lr=args.lr, max_iter=20, history_size=100, line_search_fn="strong_wolfe")
...
for epoch in range(init_epoch,int(init_epoch+num_epochs)):
    ...
    for inputs, targets in train_loader:
        ...
        loss2 = optimizer.step(closure)
        loss1,loss3 = torch.tensor(0),torch.tensor(0)
