...
class ObjectiveFormulation():
    ...
    def __init__(self, x_r=np.array([0]),device='cpu'):
        ...
        self.Q = torch.tensor([[10.,0],[0.,0.1]]).to(device)
        ...
        self.c = torch.tensor(1e6).to(device)
    ...
    def forward(self, x, nn_output):
        ...
        return obj_values.mean() + constraint1.mean() + constraint2.mean()
    ...
    def obj_fun(self,data):
        ...
        # OBJECTIVE FUNCTION
        y = self.R*u**2 + x_dag @ QQ @ (Add@x+Bdd@u-x_rr)+ (x-x_rr).permute(0,2,1)@P@(x-x_rr)
        y = y.squeeze(-1)
    ...
    def constrain1(self,x):
        ...
        return self.c*torch.sum(torch.relu(1e-15-eig_values),dim=1).unsqueeze(-1)
        # return self.c*torch.sum(torch.sign(torch.relu(1e-15-eig_values)),dim=1).unsqueeze(-1)
        # return 100*torch.sum(self.c*torch.relu(1e-15-eig_values),dim=1).unsqueeze(-1)
